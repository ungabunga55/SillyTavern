...
function migrateChatCompletionSettings(settings) {
    const migrateMap = [
        ...
        { oldKey: 'fireworks_model', oldValue: 'accounts/fireworks/models/kimi-k2-instruct', newKey: 'fireworks_model', newValue: default_settings.fireworks_model },
...
async function onModelChange() {
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.CLAUDE) {
        if (oai_settings.max_context_unlocked) {
            $('#openai_max_context').attr('max', unlocked_max);
        } else if (/^claude-(sonnet-4-5|sonnet-4-6|sonnet-5|opus-4-6|opus-4-7|opus-4-8|fable-5|mythos-5)/.test(value)) {
